...
void exercise2(){
    ...
    showEnviroment();
    Camera *cam = &CameraController::camera;
    cam->moveto(glm::vec3(0.09, 5.88, 5.56));
    CameraController::pitch = -41.30;
    CameraController::yaw = -88.55;
    cam->rotate(-41.30, -88.55);
    ...
    cube2->model = glm::scale(cube2->model, glm::vec3(1.2f, 1.2f, 1.5f));
    //cube2->scaleTo(1.2f);
    //cube3->scaleTo(1.7f);
    cube3->moveTo(glm::vec3(1.5f, 0.0f, -1.5f));
    cube3->model = glm::scale(cube3->model, glm::vec3(1.7f, 1.7f, 2.0f));
    // 地面
    Object plane(planeVertices, 6, POSITIONS_NORMALS_TEXTURES, GL_TRIANGLES);
    //plane.setShader(&sceneShader);
    plane.setCamera(&CameraController::camera);

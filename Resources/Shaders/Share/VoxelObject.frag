...
void main() {
	...
	for(int i = 0; i < LIGHTS_NUM; ++i){
		...
		// 入射光方向
		vec3 lightDir = normalize(lights_FS_in[i].position - position_FS_in);
		// 漫反射
		float diff = max(dot(normal, lightDir), 0.0);
		vec3 diffuse = lights_FS_in[i].diffuse * diff;
		// 视角方向
		...
		// 离光源距离
		float distance = length(lights_FS_in[i].position - position_FS_in);
		// 衰减
